...
# subset data
df <- subset(df, df$ss3 == 'Helix' | df$ss3 == 'Strand' | df$ss3 == 'Coil')

# reorder factor levels
df$ss3 <- factor(df$ss3, levels = c('Helix', 'Strand', 'Coil'))
...
ggplot(df, aes(phi, psi, color = ss3)) +
    ...
    # faceting
    facet_wrap(~ss3, nrow = 1, scales = 'fixed') + 
