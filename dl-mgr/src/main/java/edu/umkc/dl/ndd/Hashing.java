...
public class Hashing {
    ...
    public static Hashes hash(Integer[] grams) {

        Map<Integer,Integer> gramCounts = new HashMap<>();
        for (int i = 0; i < grams.length; i++) {
            
            Integer gram = grams[i];
            if (gramCounts.containsKey(gram)) {
                gramCounts.replace(gram, gramCounts.get(gram) + 1);
            }
            else {
                gramCounts.put(gram, 1);
            }
        }

        Integer[] minHashes = getMinHashes(gramCounts);
        Integer[] bandHashes = getBandHashes(minHashes);
    ...
    private static Integer[] getMinHashes(Map<Integer,Integer> gramCounts) {
        ...
        for (Integer key : gramCounts.keySet()) {

            // gramMap.get(key) is the count for that hash
            for (int i = 0; i < gramCounts.get(key); i++) {

                // count has to be larger than max gram
                updateMinHashes(minHashes, key + (i * 10_000_000));
            }
        }
    ...
    private static Integer[] getBandHashes(Integer[] minHashes) {
